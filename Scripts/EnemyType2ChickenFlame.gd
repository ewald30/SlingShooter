...
onready var particles = get_node("MeteoriteBurning/Area2D/CollisionShape2D/ExplosionParticles")
...
func _process(delta):
	...
	if colided == true and exploding == false:
		$MeteoriteBurning.stop()
		particles.emitting = true
		#y_speed = 1
		global.score+=1
		timer.start()
		exploding = true
		$MeteoriteBurning.play("dummy_animation")
