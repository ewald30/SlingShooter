...
onready var particles = get_node("RotatingMeteorite/Area2D/CollisionShape2D/ExplosionParticles")
...
func _process(delta):
	...
	if colided == true and exploding == false:
		...
		timer.start()
		exploding = true
		$RotatingMeteorite.stop()
		$RotatingMeteorite.play("dummy_animation")
