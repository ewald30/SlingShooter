...
onready var particles = get_node("GreenSlimeMobSprite/GreenSlimeArea2D/CollisionShape2D/ExplosionParticles")
...
func _process(delta):
	...
	if colided == true and exploding == false:
		...
		timer.start()
		exploding = true
		$GreenSlimeMobSprite.stop()
		$GreenSlimeMobSprite.play("dummy")
